...
public:
  ...
  std::vector<evalio::Point> add_lidar(evalio::LidarMeasurement mm) override {
    ...
    if (past_k_scans_.size() == 0) {
      ...
      auto scan_features = loam::extractFeatures(mm.points, *loam_lidar_params_,
                                                 loam_fe_params_);
    ...
    // Default case for all iterations except the first
      ...
      auto init_transform = past_k_scans_.size() > 0
                                ? past_k_scans_.back().first
                                : loam::Pose3d();
      auto map_T_scan = loam::registerFeatures(
          scan_features, local_map, init_transform, loam_registration_params_);
...
private:
  ...
  inline evalio::Point transform_point(evalio::Point src_pt,
                                       loam::Pose3d target_T_source) {
    auto tgt_pt = target_T_source.act(to_eigen_point(src_pt));
    return {
        tgt_pt(0), tgt_pt(1),    tgt_pt(2),  src_pt.intensity,
        src_pt.t,  src_pt.range, src_pt.row, src_pt.col,
    };
  }
  ...
  loam::LoamFeatures<evalio::Point>
  transform_features(loam::LoamFeatures<evalio::Point> features,
                     loam::Pose3d target_T_source) {
